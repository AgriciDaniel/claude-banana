...
| MCP unavailable | Fall back in this order: **1)** Replicate API (if `REPLICATE_API_TOKEN` is set): `python3 ${CLAUDE_SKILL_DIR}/scripts/replicate_generate.py --prompt "..."` **2)** Direct Gemini API (if `GOOGLE_AI_API_KEY` is set): `python3 ${CLAUDE_SKILL_DIR}/scripts/generate.py --prompt "..."`. For editing: use `replicate_edit.py` or `edit.py` respectively. |
